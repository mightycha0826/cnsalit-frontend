...
<script>
	...
	onMount(() => {
		const auth = localStorage.getItem('geumbyeol_auth');
		const profile = localStorage.getItem('geumbyeol_user');
		const path = window.location.pathname;

		const isPublicPath = path === '/' || path === '/auth/profile';

		if (!auth && !isPublicPath) {
			goto('/');
			return;
		}
		if (auth && !profile && path !== '/auth/profile') {
			goto('/auth/profile');
			return;
		}
		if (auth && profile && isPublicPath) {
			goto('/home');
			return;
		}
	...
	const showNav = $derived(page.url.pathname !== '/' && !page.url.pathname.startsWith('/auth'));

	function logout() {
		localStorage.removeItem('geumbyeol_auth');
		localStorage.removeItem('geumbyeol_user');
		goto('/');
	}
</script>
...
{#if ready}
	...
	{#if showNav}
		<nav class="navbar glass-effect">
			<div class="nav-inner">
				<a href="/dashboard" class="nav-logo" aria-label="큰별문학제 대시보드 홈">
					<svg class="logo-svg" viewBox="0 0 283.46 113.39" xml:space="preserve">
						<g>
							<path
								d="M114.21,30.27c0,1.37-0.17,2.73-0.35,3.75c-0.25,1.08-0.56,1.79-0.95,1.79c-0.42,0.04-0.67-0.67-0.81-1.54
						...
						<g>
							<path
								d="M140.39,78.84c0,1.29-0.14,2.49-0.42,3.5c-0.28,1.01-0.63,1.5-1.05,1.5c-0.42,0.04-0.63-0.49-0.77-1.29
						...
						<line class="logo-stroke" x1="120.47" y1="14.14" x2="120.47" y2="102.02" />
						<line class="logo-stroke" x1="24.11" y1="53.83" x2="259.35" y2="53.83" />
						<line class="logo-stroke" x1="85.04" y1="14.14" x2="85.04" y2="69.42" />
						<g>
							<path
								d="M68.63,27.8c-1.86,0.17-3.85,0.39-5.53,0.56c-0.7,0.04-1.23,0.25-1.79,0.42c-0.39,0.14-0.63,0.21-0.95,0.25
								...
								c1.5,0.14,3.19,0.28,6.69,0.31c1.79,0.04,4.45,0.04,7,0c2.56-0.3,4.97-0.07,6.23-1.0c0-0.42,0.21-1.57,0.46-2.73
						...
						<g>
							<path
								d="M171.71,63.69c0.25-0.1,0.53-0.21,0.84-0.25c1.16-0.14,2.52-0.14,3.5,0c0.84,0.14,1.29,0.56,1.29,1.23
						...
						<g>
							<path
								d="M205.92,69.85c0.59,0.46,1.08,0.8,1.33,0.91c0.25,0.14,0.46,0.21,0.63,0.24c0.14,0,0.32,0.04,0.46,0.04
						...
						<line class="logo-stroke" x1="155.91" y1="53.83" x2="155.91" y2="93.52" />
						<line class="logo-stroke" x1="191.34" y1="53.83" x2="191.34" y2="102.02" />
					</svg>
				</a>
				<div class="nav-links">
					<a href="/home" class:active={page.url.pathname === '/home'}>홈</a>
					<a href="/dashboard" class:active={page.url.pathname.startsWith('/dashboard')}>대시보드</a
					>
					<a href="/submit" class:active={page.url.pathname === '/submit'}>출품하기</a>
...
{:else}
	<div class="loading-screen">
		<div class="loading-logo">큰별문학제</div>
	</div>
{/if}
...
<style>
	...
	/* ── CSS Custom Properties — Starry Night over Meadow ── */
	:global(:root) {
		/* --- Starry Night over Meadow Palette --- */
		--sky-midnight: #020617;
		--sky-deep: #0f172a;
		--meadow-deep: #064e3b;
		--star-glow: #fef08a;
		--star-dim: rgba(254, 240, 138, 0.3);
		--mint-glow: #4ade80;
		--white-star: #fffdf5;

		/* Glassmorphism */
		--glass-bg: rgba(15, 23, 42, 0.6);
		--glass-border: rgba(254, 240, 138, 0.15);
		--glass-blur: blur(12px);

		/* Layout & Shadows */
		--bg-primary: var(--sky-midnight);
		--text-primary: #f8fafc;
		--text-secondary: #cbd5e1;
		--text-muted: #94a3b8;
		--shadow-star: 0 0 20px rgba(254, 240, 138, 0.2);
	...
	.stars-layer-2 {
		...
		background-image:
			...
			radial-gradient(1.5px 1.5px at 90px 40px, var(--star-glow), rgba(0, 0, 0, 0)),
	...
	.logo-svg {
		...
		width: auto;
		fill: var(--white-star);
		stroke: var(--white-star);
		filter: drop-shadow(0 0 10px rgba(254, 240, 138, 0.45));
		transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
	}

	.logo-svg:hover {
		filter: drop-shadow(0 0 15px rgba(254, 240, 138, 0.6));
		transform: scale(1.05);
	}
	...
	:global(.glass-card::before) {
		...
		right: 0;
		bottom: 0;
		background: var(--gradient-literary);
		opacity: 0.08;
		z-index: -1;
		pointer-events: none;
	}
	...
	:global(.btn-primary:hover) {
		...
		box-shadow: 0 0 25px rgba(254, 240, 138, 0.5);
	}
	:global(.btn-glass) {
